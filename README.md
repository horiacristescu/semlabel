...
semlabel calibrates each category on your own labeled examples. A confidence of 0.95 means the text scored higher than almost all known non-matches for that category, on your data. `show` tells you how often a category is right on examples it was not trained on, and when a label is wrong you can see the examples behind it and fix it with `add`.

Jev's probabilities are calibrated by TypeSafe on synthetic training data, and the model can't be tuned on yours ([docs](https://docs.typesafe.ai/models)). Independent tests found them good on public benchmarks but off on unfamiliar tasks, overconfident on some question types and underconfident on others ([study](https://github.com/scienthoon/jev-ood-calibration), [audit](https://github.com/jujumilk3/jev-calibration-audit)). The setups that worked best added the user's own labels and a small model on top ([example](https://www.beri.net/article/typesafe-jev-typed-decision-model-calibration-decomposition-shadow-eval)), which is where semlabel starts.
